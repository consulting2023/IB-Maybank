...
export default class TransferenciaOutros extends Component {
    ...
    valor_tarifa = (valor_simulacao) => {
        ...
        const data = {
            url: 'tarifa/consulta',
            data: { 'conta_id': this.state.pessoa.conta_id, 'chave': 'transferencia', 'valor': valor_simulacao },
            method: 'POST',
        };
  ...
  combinacoes = async () => {
    ...
    comprovante_ver = (id) => {
        ...
        Funcoes.Geral_API(data, true).then((responseJson) => {
          ...
          setTimeout(() => {
            this.setState({ titleModalComprovante: 'Transferência efetuada com sucesso!', loading: false });
          }, 1000);
        });
    };
    ...
    render() {
        ...
        const tipoContas = [
            ...
            { label: i18n.t('transferencia.contaCorrente'), value: 'Conta corrente', key: 1 },
            // { label: 'Conta poupança', value: 'Conta poupança', key: 2 },
            { label: i18n.t('transferencia.contaPoupanca'), value: 'Conta poupança', key: 2 },
        ];
            ...
            return (
                <>
                    ...
                    <Container className="mt-1 col-md-12 text-center">
                        <p className="text-center" style={{ fontSize: '1.85em' }}>{i18n.t('transferencia.titleTransfOutros')}</p>
                    </Container>
                    <Container className="mt-4 campoGerarTransf col-md-10 margin-auto" style={{}} >
                        <Col >
                            ...
                            <div className="margin-auto">
                                ...
                                <Row>
                                    <div className="form-group col-12">
                                        ...
                                        <Row>
                                            <Button variant="outline-primary" style={{width: '80px', height: '80px'}} className="iconeBancos" onClick={() => {
                                                this.state.bancos.map((item, index) => {
                                                    if (item.id == '000') {
                                                        this.setState({ selecionarRefBanco: item.title, id_banco: item.id })
                                                    }
                                                })
                                            }}>
                                            ...
                                            <Button variant="outline-primary" style={{width: '80px', height: '80px'}} className="iconeBancosOutros" onClick={() => {
    
                                                this.setState({ mostrar_pesquisar: true })
    
                                            }}> <p style={{ width: '100%', heigth: '100%' }}>{i18n.t('transferencia.textOutrosBancos')}</p>
                                            </Button>
                                        </Row>
                                        <label></label>
                                        <Row>
                                ...
                                <Row>
                                    ...
                                    <div className="form-group col-2">
                                        ...
                                        <input
                                            ...
                                            onChange={(e) => this.setState({ digitoAg: e.target.value })}
                                        />
                                    </div>
                                </Row>
                                <Row>
                                    ...
                                    <div className="form-group col-7">
                                        ...
                                        <select required className="form-control" value={this.state.tipoConta} onChange={(e) => this.setState({ tipoConta: e.target.value })} >
                                            <option>{i18n.t('transferencia.selecTipoConta')}</option>
                                            {tipoContas.map(tipo => (
                                                <option key={indice_bancos++} value={tipo.value}>{tipo.label}</option>
                                            ))}
                                        </select>
                                    </div>
                                </Row>
                                <Row>
                                    ...
                                    <div className="form-group col-8">
                                        <label>{i18n.t('transferencia.favorecidoTransfOutros')}</label>
                                        <input
                                            className="form-control"
                                            required
                                            id="favorecido"
                                            ...
                                            onChange={(e) => this.setState({ favorecido: e.target.value })}
                                        />
                                    </div>
                                </Row>
                                <Row>
                                    <div className="form-group col-4">
                                        ...
                                        <CurrencyInput
                                            ...
                                            onChange={(valor) => this.setState({ valor: valor, valor_simulacao: valor })}
                                        />
                                    </div>
    
                                    <div className="form-group col-8">
                                        <label>{i18n.t('transferencia.finalidadeTransfOutros')}</label>
                                        <select required className="form-control" value={this.state.finalidade} onChange={(e) => this.setState({ finalidade: e.target.value })}>
                                            <option>{i18n.t('transferencia.selectFinalidadeTransfOutros')}</option>
                                            {items.map(item => (
                                                <option key={indice_bancos++} value={item.value}>{item.label}</option>
                                            ))}
                                        </select>
                                    </div>
                                </Row>
                                <br />
                                <p className="textDescr" style={{}}><Icon.InfoCircle /> <i>{i18n.t('transferencia.tranfDescrOutros')}</i></p>
                                ...
                                <br></br>
                                <button onClick={() => { /* this.transferencia();  */this.setState({ disabilitar: true, modal_confirmacao: true }) }} className="btn realizarTransferencia btn-sm btn-success">{i18n.t('transferencia.btnTransferir')}</button>
                            </div>
                        </Col>
                    </Container>
    
                    <Modal centered size='lg' animation={true} show={this.state.modal_confirmacao} onHide={() => { this.setState({ modal_confirmacao: false }) }}>
                        <Modal.Body>
                            <Container  >
                                ...
                                <Row>
                                    ...
                                    <h4 style={{ fontWeight: '600' }}>&nbsp;&nbsp; {this.state.selecionarRefBanco}</h4>
                                </Row>
    
                                <Row>
                                    <h4>{i18n.t('transferencia.agenciConfirmeDados')}</h4>
                                    <h4 style={{ fontWeight: '600' }}>&nbsp;&nbsp; {this.state.agencia}</h4>
                                </Row>
    
                                {this.state.digitoAg != '' ? (
                                    <Row>
                                        <h4>{i18n.t('transferencia.digitoConfirmeDados')} </h4>
                                        <h4 style={{ fontWeight: '600' }}>&nbsp;&nbsp; {this.state.digitoAg}</h4>
                                    </Row>
                                ) : null}
    
                                <Row>
                                    <h4>{i18n.t('transferencia.contaConfirmaDados')}</h4>
                                    <h4 style={{ fontWeight: '600' }}>&nbsp;&nbsp; {this.state.conta}</h4>
                                </Row>
    
                                <Row>
                                    <h4>{i18n.t('transferencia.digitoConfirmeDados')} </h4>
                                    <h4 style={{ fontWeight: '600' }}>&nbsp;&nbsp; {this.state.digito}</h4>
                                </Row>
    
                                <Row>
                                    <h4>{i18n.t('transferencia.tipoContaConfirmeDados')}</h4>
                                    <h4 style={{ fontWeight: '600' }}>&nbsp;&nbsp; {this.state.tipoConta}</h4>
                                </Row>
    
                                <Row>
                                    <h4>{i18n.t('transferencia.documentoConfirmeDados')}</h4>
                                    <h4 style={{ fontWeight: '600' }}>&nbsp;&nbsp;{this.state.documento}</h4>
                                </Row>
    
                                <Row>
                                    <h4>{i18n.t('transferencia.favorecidoConfirmeDados')}</h4>
                                    <h4 style={{ fontWeight: '600' }}>&nbsp;&nbsp;{this.state.favorecido}</h4>
                                </Row>
    
                                <Row>
                                    <h4>{i18n.t('transferencia.valorConfirmeDados')}</h4>
                                    <h4 style={{ fontWeight: '600' }}>&nbsp;&nbsp;R$ {this.state.valor}</h4>
                                </Row>
    
                                <hr />
    
                                <button type="button" className="btn btnTransfCancelar" onClick={() => this.setState({ modal_confirmacao: false })}>{i18n.t('transferencia.btnCancelar')}</button>
                                <button onClick={() => this.transferencia()} className="btn btnTransfSucesso float-right btnTransfSucesso">{i18n.t('transferencia.btnTransferir')}</button>
                    ...
                    <Modal centered animation={true} show={this.state.mostrar_pesquisar} onHide={() => { this.setState({ mostrar_pesquisar: false }) }}>
                        <Modal.Body>
                            <Container className="text-center" >
                                <h5>{i18n.t('transferencia.selectBanco')}</h5>
                                <div style={{ margin: '0 auto', marginTop: '10%', textAlign: 'left' }}>
                                    <label >{i18n.t('transferencia.pesquisarBanco')} &nbsp;</label>
                                    <input style={{ width: '70%' }} type="text" onChange={(event) => this.pesquisar(event)} />
                                </div>
                                <hr />
                                <ul className="listasBancosOutros">
                                    {this.state.bancos_filtro.map((item, index) => {
                                        return (
                                            <li>
                                                <Button className="bancosOutros" key={index} onClick={() => { this.setState({ selecionarRefBanco: item.title, id_banco: item.id, mostrar_pesquisar: false }) }}>
                                                    <label className="titlebancos" value={item.id}>{item.title.toUpperCase()}</label>
                                                </Button>
                                            </li>
                                        )
                                    })}
                        ...
                        <Modal.Body className="modalValidacao">
                            ...
                            <Container>
                                <Row className="justify-content-center">
                                    ...
                                    <button type="button" className="btn btn-primary col-12 mt-12 text-center tokenButton"
                                        style={{ height: 70, justifyContent: 'center' }}
                                        onClick={() => this.input_Token(1)}>
                                        <span>{Icones.token}<h5>{i18n.t('transferencia.tokenValidacao')}</h5></span>
                                    </button>
                                ...
                                <Row className="justify-content-center">
                                    {this.state.loading ? (
                                        ...
                                        <button type="button" className="btn btn-primary col-5 mt-12 text-center tokenButton"
                                            ...
                                            <span>{Icones.pagarConta}<h5>{i18n.t('transferencia.btnTransferir')}</h5></span>
                                        </button>
                                    )}
