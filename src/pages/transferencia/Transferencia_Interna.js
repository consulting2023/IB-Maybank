...
import { Container, Col, Button, Row, Modal, Image, Alert } from 'react-bootstrap';
...
export default class TransferenciaInterna extends Component {
  ...
  selecionarContaTelefone = (dados) => {
    ...
    this.setState({ selecionarConta: dados, contaTelefone: false, contaSelecionada: true })
  };
  ...
  closeModalConta = () => {
    this.setState({ conta: false, buscarConta: '', buscarDigito: '', agenciaBusca: '' });
  }
      ...
      <div>
        ...
        <Container className="p-3 d-flex justify-content-center">
          <Col md={8} className="baseWindow px-5 py-4">
            <Row>
              <p className="mb-2 w-100 text-center" style={{ fontSize: "1.30em" }}><strong>{i18n.t('transferencia.textInterna', { nome_banco: process.env.NOME_BANCO })}</strong></p>
            </Row>
            <Row className="text-center px-3">
              <Col className="p-3">
                <Button variant="outline-primary" className="baseButtonPrimary" onClick={() => this.setState({ conta: true })}>
                  <Row className="w-80 m-auto">
                    <Col xs={5} className="m-auto px-0">
                      {Icones.transfBuscaConta}
                    </Col>
                    <Col xs={7} className="px-0 my-auto">
                      <p className="buttonTitle m-auto">{i18n.t('transferencia.btnBuscarConta')}</p>
                    </Col>
                  </Row>
                </Button>
              </Col>
              <Col className="p-3">
                <Button variant="outline-primary" className="baseButtonPrimary" onClick={() => this.setState({ telefone: true })}>
                  <Row className="w-80 m-auto">
                    <Col xs={5} className="m-auto px-0">
                      {Icones.transfBuscaTelefone}
                    </Col>
                    <Col xs={7} className="px-0 my-auto">
                      <p className="buttonTitle m-auto">{i18n.t('transferencia.btnBuscarTelefone')}</p>
                    </Col>
                  </Row>
                </Button>
              </Col>
              <Col className="p-3">
                <Button variant="outline-primary" className="baseButtonPrimary" onClick={() => this.setState({ cpf: true })}>
                  <Row className="w-80 m-auto">
                    <Col xs={5} className="m-auto px-0">
                      {Icones.transfBuscaCpf}
                    </Col>
                    <Col xs={7} className="px-0 my-auto">
                      <p className="buttonTitle m-auto">{i18n.t('transferencia.btnBuscarDocumento')}</p>
                    </Col>
                  </Row>
                </Button>
              </Col>
        ...
        <Modal centered size="sm" show={this.state.telefone} onHide={() => this.closeModalTelefone() }>
          ...
          <Modal.Body className="py-3 px-5">
            <div>
              <div className="form-group">
                <label className="mb-2">{i18n.t('transferencia.textTelefone')}</label>
            {/* {i18n.t('transferencia.btnBuscarTelefone')} */}
                <InputMask
                  onChange={(e) => this.setState({ telefoneBusca: e.target.value })}
          ...
          <Modal.Footer className="">
            <Button onClick={() => this.buscaTelefone()} className="rounded-pill">{i18n.t('transferencia.btnPesquisar')}</Button>
          </Modal.Footer>
        </Modal>

        <Modal centered size="sm" show={this.state.cpf} onHide={() => this.closeModalCpf() }>
          <Modal.Header closeButton>
          ...
          <Modal.Body className="py-3 px-5">
            <div>
              <div className="form-group">
                <label className="mb-2">{i18n.t('transferencia.textDocumento')}</label>
                <InputMask
                  className="form-control"
                  id="cpfBusca"
                  required
          ...
          <Modal.Footer>
            <Button onClick={() => this.buscarCpf()} className="rounded-pill">{i18n.t('transferencia.btnPesquisar')}</Button>
          </Modal.Footer>
        </Modal>

        <Modal centered size="sm" show={this.state.conta} onHide={() => this.closeModalConta() }>
          <Modal.Header closeButton>
            ...
            <div>
              <div className="form-group">
                ...
                <input
                  ...
                  type={'number'}
                  onChange={(e) => this.setState({ agenciaBusca: e.target.value })}
                />
              </div>
              <div className="form-group">
                ...
                <input
                  ...
                  type={'number'}
                  onChange={(e) => this.setState({ buscarConta: e.target.value })}
                />
              </div>
              <div className="form-group">
                ...
                <input
                  ...
                  type={'number'}
                  onChange={(e) => this.setState({ buscarDigito: e.target.value })}
                />
              </div>
            </div>
          </Modal.Body>
          <Modal.Footer>
            <Button onClick={() => this.buscarConta()} className="rounded-pill">{i18n.t('transferencia.btnPesquisar')}</Button>
          </Modal.Footer>
        </Modal>

        <Modal centered animation={true} show={this.state.contas} onHide={() => { this.setState({ contas: false }), window.location.href = '/transferencia_interna' }}>
          <Modal.Body>
            <Container className="text-center" >
              <h5>{i18n.t('transferencia.textSelecionarConta')}</h5>
              {
                this.state.contasMultiplas.map(conta => {
                  if (parseInt(this.state.pessoa.conta_id) != parseInt(conta.correntista.conta.id)) {
                    return (
                      <button
                      ...
                      >
                        {i18n.t('transferencia.nomeConta')} {conta.correntista.dados.nome || conta.correntista.dados.razao_social}
                        <br></br>
                        {i18n.t('transferencia.agenciaConta')} {conta.correntista.conta.agencia}
                        <br></br>
                        {i18n.t('transferencia.contaConta')}{("0000000" + conta.correntista.conta.id).slice(-7)}-{conta.correntista.conta.digito}
                        <br></br>

                        {conta.correntista.dados.mostrar ? (
                          <Image style={{ width: "50px", height: "50px", float: 'right', margin: '-56px 0 0 0' }} src={'data:image/png;base64,' + conta.correntista.dados.imagem} roundedCircle />
                        ) : (
              ...
              <Col>
                <button 
                  ...
                  onClick={() => { this.setState({ contas: false }), window.location.href = '/transferencia_interna' }}
                >
        ...
        <Modal centered animation={true} show={this.state.contaTelefone} onHide={() => { this.setState({ contas: false }), window.location.href = '/transferencia_interna' }}>
          <Modal.Body>
            <Container className="text-center" >
              <h5>{i18n.t('transferencia.textSelecionarConta')}</h5>
                  ...
                  if (parseInt(this.state.pessoa.conta_id) != parseInt(conta.correntista.conta.id)) {
                    return (
                      <button
                        ...
                        className="btn btn-outline-secondary col-10 mt-1 text-left"
                        style={{ fontSize: '1.00em', color: "black" }}
                        onClick={() => this.selecionarContaTelefone(conta.correntista)}
                      >
                        {i18n.t('transferencia.nomeConta')} {conta.correntista.dados.nome || conta.correntista.dados.razao_social}
                        <br></br>
                        {i18n.t('transferencia.agenciaConta')} {conta.correntista.conta.agencia}
                        ...
                        {conta.correntista.dados.CPNJ ? (
                          ...
                          <Image 
                            style={{ width: "50px", height: "50px", float: 'right' , margin: '-34px 0 0 0'}}
                            src={'data:image/png;base64,' + conta.correntista.dados.imageself}
                            roundedCircle
                          />
                        )}
                      </button>
              ...
              <Col>
                <button
                  type="button"
                  className="btn btnTransfCancelar float-right mt-3" 
                  onClick={() => { this.setState({ contaTelefone: false }), window.location.href = '/transferencia_interna' }}
                >
                  {i18n.t('transferencia.btnFechar')}
                </button>
              </Col>
        ...
        <Modal centered animation={true} size="sm" show={this.state.contaSelecionada} onHide={() => { this.setState({ contaSelecionada: false }), window.location.href = '/transferencia_interna' }}>
          <Modal.Body>
            <Alert style={{ fontSize: '13px' }} variant="secondary">
              <strong><i>{i18n.t('transferencia.textDadosFavorecido')}</i></strong>
              <br></br>
              {i18n.t('transferencia.nomeConta')} {this.state.selecionarConta.dados.nome || this.state.selecionarConta.dados.razao_social}
              <br></br>
              {i18n.t('transferencia.agenciaConta')} {this.state.selecionarConta.conta.agencia} &nbsp; {i18n.t('transferencia.contaConta')}000000{this.state.selecionarConta.conta.id}-{this.state.selecionarConta.conta.digito}
              <br></br>
            </Alert>
            <hr></hr>
            <div className="showhimTransf">
              <div className="hiddenmeTransf" >
                <p className="saldos-home texto-saldos-Transf" >{i18n.t('transferencia.saldo')} <Icon.Eye className="ocultarSaldoTransf" style={{ fontWeight: '500' }} /></p>
              </div>
              <div className="showmeTransf">
                <p className="saldos-home texto-saldos-Transf" >{i18n.t('transferencia.saldo')} {Formatar.formatarMoeda(this.state.saldoDigital)}</p>
              </div>
            </div>
            <div>
              <div className="form-group ">
                ...
                <CurrencyInput
                  ...
                  onChange={(valor) => this.setState({ valorTransferencia: valor })}
                />
              </div>
              <button
                onClick={() => this.setState({ token_app: true })}
              ...
              <button
                ...
                onClick={() => { this.setState({ contaSelecionada: false }), window.location.href = '/transferencia_interna' }}
              >
                {i18n.t('transferencia.btnFechar')}
              </button>
            </div>
        ...
        <Modal
          ...
          onHide={() => { this.setState({ token_app: false }), window.location.href = '/transferencia_interna' }}
        >
          <Modal.Body className="modalValidacao">

            <h2>{i18n.t('transferencia.titleModalValidacao')}</h2>
            <hr />
            <h5>{i18n.t('transferencia.textModalValidacao', { nome_banco: process.env.NOME_BANCO })}</h5>
            <br />
            <Container>
              <Row className="justify-content-center">
                ...
                >
                  <span>
                    {Icones.token}<h5>{i18n.t('transferencia.tokenValidacao')}</h5>
                  </span>
                </button>
              </Row>
              ...
              <Row className="justify-content-center">
                {
                  this.state.loading ? (
                    ...
                    >
                      <span>
                        {Icones.TransfConta}<h5>{i18n.t('transferencia.btnTransferir')}</h5>
                      </span>
                    </button>
                )}
              </Row>
