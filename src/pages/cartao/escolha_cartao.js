...
export default class EscolhaCartao extends Component {
    ...
    componentDidMount = () => {
        ...
        Funcoes.Geral_API(data, true).then((res) => { this.setState({ saldoDigital: res.digital, saldoCartao: res.cartao }) });

        // tarifas
        const data2 = {
            url: 'tarifa/consulta',
            data: { 'conta_id': pessoa.conta_id, 'chave': 'transferencia', 'valor': 'solicitacao_cartao' },
            method: 'POST',
        };
    ...
    cartao = () => {
        ...
        if (this.state.pessoa.pf_pj == 'pf') {
            ...
            if (doc.length == 11) {
                this.setState({ doc: Formatar.cpf_mask(this.state.pessoa.doc), texto: 'CPF:' })
            } else if (doc.length == 14) {
                this.setState({ doc: Formatar.cnpj_mask(this.state.pessoa.doc), texto: 'CNPJ: ' })
            }
        }

        if (cartao_status[0]) {
            if (cartao_status.dataHoraDesbloqueio == null || cartao_status.dataHoraDesbloqueio == '') {
    ...
    solicitar_cartao = () => {
        ...
        if (parseFloat(this.state.saldoDigital) < parseFloat(valor_cartao)) {
            alert('Saldo insuficiente, ' + 'Realize uma recarga em sua conta digital para continuar a operação.')
        } else if (this.state.checked != true) {
            alert('Você precisa aceitar os termos de uso para solicitar um cartão');
        } else {
            Funcoes.novoCartao(dados).then((res) => {
                var nome = this.state.pessoa.nome;
                if (res == 0) {
                    alert(nome + ', Ocorreu um erro ao solicitar o cartão tente novamente')
                } else if (res == 1) {
                    alert(nome + ', Seu cartão foi solicitado com sucesso em breve recebera um email com as novidades')
                } else if (res == 2) {
                    alert(nome + ', Já existe uma solicitação do cartão, aguarde em breve recebera um email com as novidades')
                }
    ...
    ativacao = () => {
        ...
        if (this.state.cartao_status[0]) {
            ...
            alert('Para ativar seu cartão ' + process.env.NOME_BANCO + ' primeiro você precisa solicitar um cartão.')
    ...
    carregar = () => {
        ...
        } else if (this.state.cartao_status.dataHoraDesbloqueio == null || this.state.cartao_status.dataHoraDesbloqueio == '') {
            alert('Para carregar seu cartão ' + process.env.NOME_BANCO + ' primeiro você precisa ativalo.')
        } else {
            alert('Para carregar seu cartão ' + process.env.NOME_BANCO + ' primeiro você precisa solicitar um cartão.')
        }
    }
    ...
    senha = () => {
        if (this.state.cartao_status[0]) {
            ...
            alert('Para trocar a senha de seu cartão ' + process.env.NOME_BANCO + ' primeiro você precisa ativa-lo.')
        } else {
            alert('Para trocar a senha de seu cartão ' + process.env.NOME_BANCO + ' primeiro você precisa solicitar um cartão.')
        }
    }
    ...
    detalhes = () => {
        if (this.state.cartao_status[0]) {
            ...
            alert('Para visualizar os detalhes de seu cartão ' + process.env.NOME_BANCO + ' primeiro você precisa ativa-lo.')
        } else {
            alert('Para visualizar os detalhes de seu cartão ' + process.env.NOME_BANCO + ' primeiro você precisa solicitar um cartão.')
        }
    }
    ...
    CodigoValidacao_ciacard = () => {
        ...
        Funcoes.validacaoCiaCard(dados).then((res) => {

            var nome = this.state.pessoa.razao_social;
            if (res == 1) {
                this.setState({ confirma_sms: true, cadastro_ativo: false, aguarde: false })
            } else {
    ...
    validaCodigoCiacard = () => {
        if (this.state.sms != '') {
            ...
            Funcoes.validaCodigoCia(dados).then((res) => {
                var nome = this.state.pessoa.razao_social;
                if (res == 1) {
    ...
    GerarSenha_ciacard = () => {
        Funcoes.gerarSenhaCiacard().then((res) => {
            var nome = this.state.pessoa.razao_social;
            if (res !== 0 && this.state.pessoa.pf_pj == 'pf') {
                this.setState({ confirmar_id_cartao: true, confirma_sms: false, aguarde: false })
    ...
    desbloqueio_cartao_ciacard = () => {
        ...
        var nome = this.state.pessoa.razao_social;
        Funcoes.desbloquearCartao(dados).then((res) => {
    ...
    gerar_senha_cartao = () => {
        ...
        if (this.state.senha == '') {
            ...
            Funcoes.gerarSenhaCartao(dados).then((res) => {
    ...
    salvar_desbloqueio_cartao = () => {
        Funcoes.cartaoDesbloqueio().then((res) => {
            this.setState({ gerar_senha: false, desbloqueio_efetuado: true, cartao_desbloqueado: true, });
    ...
    consulta_interna_cartao = () => {
        Funcoes.ConsultaCartao().then((res) => {
    ...
    gerar_boleto = () => {
        ...
        if (this.state.advanced == 0 || this.state.advanced == '') {
            ...
            Funcoes.boletoCartao(dados).then((res) => {
                ...
                if (res.id) {
                    ...
                    global.boleto = {
                        ...
                        instrucoes: 'Recarga ' + process.env.NOME_BANCO + ' - Receber o valor informado',
                        /* juros:res.juros, */
    ...
    gerar_pdf_boleto = (id) => {
        ...
        Funcoes.pdfCartao(dados).then((res) => {
    ...
    transf_cartao = (id) => {
        ...
        if (this.state.id_cartaoTransf == '') {
            ...
            this.setState({ mostrar_valor: false, cancelar_buscar: true })
            alert(nome + ", Código do cartão destino não pode ser igual ao código do cartão origem.")
        } else {
            const dados = {
                'id_cartao': id
            }
            ...
            Funcoes.cartao2p2(dados).then((res) => {
    ...
    gerar_pagamennto = () => {
        ...
        if (valor_conta == '') {
            ...
            Funcoes.Valida_senha(dados).then((res) => {
                if (res == 0) {
                    this.setState({ loading: false, carregar: true })
    ...
    transf_enviar = () => {
        ...
        Funcoes.cartaoTransfEnviar(dados).then((res) => {
    ...
    alterar_senha = () => {
        if (this.state.nova_senha == '') {
            ...
            const dados = this.state.nova_senha



            Funcoes.cartaoTrocarSenha(dados).then((res) => {
        ...
        Funcoes.cartao_dados().then((res) => {
            ...
            if (res[0].id_status) {
                ...
                for (var i = 0; i < tamanho_keys; i++) {
                    ...
                    if (res[i].id_account == this.state.cartao_status[0].id_conta_cartao) {
    ...
    desbloquear_bloquear_cartao = (id) => {
        if (id == 1) {
            ...
            Funcoes.bloquear(dados).then((res) => {
            ...
            const dados = this.state.cartao_status[0].id_cartao;

            Funcoes.desbloquear(dados).then((res) => {
        ...
        return (
            <>
                ...
                <section >
                    ...
                    <Container fluid className="mt-4">
                        ...
                        <Col className="text-center">
                            ...
                            <Button
                                ...
                                className="btn btn-primary botoesCartao"
                                style={{}} >
                            ...
                            <Button
                                ...
                                <Icon.JournalText className="iconCartao"
                                    style={{}} />
                                <p className="tituloBotoes">
                                    <br></br>
                                    {/* Ativar Cartão */}
                                    {i18n.t('escolher_cartão.ativar')}
                                </p>
                            </Button>
                            {/* Recarga */}
                            <Button
                                ...
                                className="btn btn-primary botoesCartao"
                                style={{}} >
                                <Icon.ArrowLeftRight className="iconCartao"
                                    style={{}} />
                                <p className="tituloBotoes">
                                    <br></br>
                                    {/* Carregar Cartão */}
                            ...
                            <Button
                                ...
                                className="btn btn-primary botoesCartao"
                                style={{}} >
                                <Iconfa.FaRegKeyboard className="iconCartao"
                                    style={{}} />
                                <p className="tituloBotoes">
                                    <br></br>
                                    {/* Trocar Senha */}
                            ...
                            <Button
                                ...
                                className="btn btn-primary botoesCartao"
                                style={{}} >
                                <Icon.FileEarmarkArrowUp className="iconCartao"
                                    style={{}} />
                                <p className="tituloBotoes">
                                    <br></br>
                                    {/* Detalhes */}
                    ...
                    {/* Modal Solicitação */}
                    <Modal centered animation={true} size="lg" show={this.state.solicitar} onHide={() => window.location.href = '/cartoes'} >
                        <Modal.Body>
                            <div>
                                <Alert style={{ fontSize: '13px' }} variant="secondary">
                                    ...
                                    <p>
                                        ...
                                        {i18n.t('escolha_cartao.aquisicaoDesc', {nome_banco: process.env.NOME_BANCO, tarifa: Formatar.formatReal(this.state.tarifa)})}
                                ...
                                <h6>{i18n.t('escolha_cartao.selecao', {nome_banco:  process.env.NOME_BANCO})}</h6>

                                <InputGroup className="mb-3">
                                    <InputGroup.Prepend>
                                        <InputGroup.Radio checked={this.state.cartao1} style={{ backgroundColor: 'transparent' }} onChange={() => this.corCartao(1)} />&nbsp;&nbsp; {process.env.CARTAO1}
                                    </InputGroup.Prepend>
                                    &nbsp;&nbsp;&nbsp;
                                    <InputGroup.Prepend>
                                        <InputGroup.Radio checked={this.state.cartao2} onChange={() => this.corCartao(2)} />&nbsp;&nbsp; {process.env.CARTAO2}
                                    </InputGroup.Prepend>
                                </InputGroup>


                                {/* <Button className="termosCartao" onClick={() => this.termos(1)}>Leia os termos de uso</Button> &nbsp;&nbsp; */}
                                <Button className="termosCartao" onClick={() => this.termos(1)}>{i18n.t('escolha_cartao.termos')}</Button> &nbsp;&nbsp;
                                <Button className="termosCartao" onClick={() => this.termos(2)}>{i18n.t('escolha_cartao.termos')}</Button>

                                <br /><br />
                                <Alert variant="info" style={{ backgroundColor: "transparent" }}>
                                    <InputGroup className="mb-3" style={{ margin: '10px 0' }}>
                                        <InputGroup.Prepend>
                                            <InputGroup.Checkbox style={{ width: '25px', height: '25px' }} checked={this.state.checked} onChange={() => this.setState({ checked: !checked })} /><p style={{ margin: 'auto' }}>Declaro que li e aceito os termos de uso e de privacidade {process.env.NOME_BANCO}</p>
                                        </InputGroup.Prepend>
                                    </InputGroup>
                                </Alert>
                                <Button onClick={() => { this.setState({ solicitar: false }); window.location.href = '/cartoes' }} className="btn-danger btnCartaoCancelar" >Cancelar</Button>
                                <Button className="btnCartaoSucesso" style={{ float: 'right' }} onClick={() => this.solicitar_cartao()}>Solicitar</Button>
                            </div>
                        </Modal.Body>
                    </Modal>
                    {/* Modal Ativação */}
                    <Modal centered animation={true} size="lg" show={this.state.ativar} onHide={() => window.location.href = '/cartoes'}   >
                        <Modal.Body>

                            <Alert variant="secondary">
                                <h6>
                                    {/* Para ativar seu cartão  */}
                                    {i18n.t('escolha_cartao.confirme', {nome_banco: process.env.NOME_BANCO})}
                                    {/* {process.env.NOME_BANCO} */}
                                ...
                                <span>{this.state.texto}  {this.state.doc}</span> <br />
                                <span>
                                    {/* Telefone:  */}
                            ...
                            <p className="textDescr" ><Icon.InfoCircle /> 
                                <i>
                                    {/* Caso os dados estejam incorretos, entre em contato com o suporte  */}
                                    {i18n.t('escolha_cartao.textoDescricao', {nome_banco: process.env.NOME_BANCO, email_banco: process.env.EMAIL_BANCO})}
                                    {/* {process.env.NOME_BANCO} */}
                                     {/* no  */}
                            ...
                            {this.state.cadastro_ativo ? (
                                <div>
                                    <br />
                                    <Button onClick={() => { this.setState({ ativar: false }); window.location.href = '/cartoes' }} className="btn-danger btnCartaoCancelar" >Cancelar</Button>
                                    <Button className="btnCartaoSucesso" onClick={() => this.CodigoValidacao_ciacard() /* this.setState({ cadastro_ativo: false, confirma_sms: true }) */} style={{ float: 'right' }}>Continuar</Button>
                                </div>
                            ) : null}
                            ...
                            {this.state.confirma_sms ? (
                                <div>
                                    ...
                                    <input ref={(input) => { this.sms = input; }} type="text" value={this.state.sms} onChange={(e) => this.setState({ sms: e.target.value })} />
                                    <br /><br />
                                    <Button onClick={() => { this.setState({ ativar: false }); window.location.href = '/cartoes' }} className="btn-danger btnCartaoCancelar" >Cancelar</Button>
                                    <Button className="btnCartaoSucesso" onClick={() => this.validaCodigoCiacard()  /* this.setState({ confirma_sms: false, confirmar_id_cartao: true }) */} style={{ float: 'right' }}>Continuar</Button>
                                </div>
                            ) : null}
                            ...
                            {this.state.confirmar_id_cartao ? (
                                <div>
                                    <h6>
                                        {i18n.t('escolha_cartao.instrucao')}
                                    </h6>
                                    <input type="text" value={this.state.id_cartao} onChange={(e) => this.setState({ id_cartao: e.target.value })} style={{ width: '100%' }} ref={(input) => { this.id_cartao = input }} />
                                    <br /><br />
                                    <Button onClick={() => { this.setState({ ativar: false }); window.location.href = '/cartoes' }} className="btn-danger btnCartaoCancelar" >Cancelar</Button>
                                    <Button className="btnCartaoSucesso" onClick={() => this.desbloqueio_cartao_ciacard()   /* this.setState({ confirmar_id_cartao: false, confirma_id_cartao_pj: true }) */} style={{ float: 'right' }}>Continuar</Button>
                                </div>
                            ) : null}
                            ...
                            {this.state.confirma_id_cartao_pj ? (
                                <div>
                                    <h6>
                                        ...
                                        {i18n.t('escolha_cartao.digite', {nome_banco: process.env.NOME_BANCO})}
                                        {/* {process.env.NOME_BANCO} */}
                                    </h6>
                                    <input type='text' value={this.state.id_cartao} onChange={(e) => this.setState({ id_cartao: e.target.value })} ref={(input) => { this.id_cartao = input; }} />
                                    <br /><br />
                                    <Button onClick={() => { this.setState({ ativar: false }); window.location.href = '/cartoes' }} className="btn-danger btnCartaoCancelar" >Cancelar</Button>
                                    <Button className="btnCartaoSucesso" onClick={() => this.desbloqueio_cartao_ciacard()   /*  this.setState({ confirma_id_cartao_pj: false, gerar_senha: true }) */} style={{ float: 'right' }}>Continuar</Button>
                                </div>
                            ) : null}
                            ...
                            {this.state.gerar_senha ? (
                                <div>
                                    <h6>
                                        ...
                                        {i18n.t('escolha_cartao.crieSenha', {nome_banco: process.env.NOME_BANCO})}
                                        {/* {process.env.NOME_BANCO} */}
                                    </h6>
                                    <input type='password' maxLength={4} value={this.state.senha} onChange={(e) => this.setState({ senha: e.target.value })} ref={(input) => { this.senha = input; }} /> <br /><br />
                                    <input type='password' maxLength={4} value={this.state.confirmar_senha} onChange={(e) => this.setState({ confirmar_senha: e.target.value })} ref={(input) => { this.confirmar_senha = input; }} />
                                    <br /><br />
                                    <Button onClick={() => { this.setState({ ativar: false }); window.location.href = '/cartoes' }} className="btn-danger btnCartaoCancelar" >Cancelar</Button>
                                    <Button className="btnCartaoSucesso" onClick={() => this.gerar_senha_cartao()  /* this.setState({ gerar_senha: false, desbloqueio_efetuado: true }) */} style={{ float: 'right' }}>Continuar</Button>
                                </div>
                            ) : null}
                            ...
                            {this.state.desbloqueio_efetuado ? (
                                <div>
                                    <h6>
                                        {/* Parabéns! Seu cartão  */}
                                        {i18n.t('escolha_cartao.desbloqueio', {nome_banco: process.env.NOME_BANCO})}
                                        {/* {process.env.NOME_BANCO} */}
                                         {/* foi desbloqueado com sucesso. Aproveite!! */}
                                         {/* {i18n.t('escolha_cartao.desbloqueio2')} */}
                                    </h6>
                                    <Button className="btnCartaoSucesso" onClick={() => { this.setState({ ativar: false }); window.location.href = '/cartoes' }} style={{ float: 'right' }} variant="success" >Concluir</Button>
                                </div>
                            ) : null}
                    ...
                    {/* Modal Recarga */}
                    <Modal centered animation={true} size="lg" show={this.state.carregar} onHide={() => window.location.href = '/cartoes'} >
                        <Modal.Body>
                            <div>
                                <h5>
                                ...
                                <Container>
                                    <Row className="justify-content-center">
                                        <Button className="btn btnCartaoRecarga btn-primary col-5 mt-12 text-center " style={{ justifyContent: 'center' }} size="lg" variant="success" onClick={() => this.setState({ boleto_recarga: true, transf_recarga: false })}>Boleto</Button>
                                        &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                                        <Button style={{ justifyContent: 'center' }} size="lg" className="btn btnCartaoRecarga btn-primary col-5 mt-12 text-center " variant="success" onClick={() => {
                                            this.setState({ mostrar_valor: false, transf_recarga: true, boleto_recarga: false })
                                        }}>
                                            {/* Transferencia */}
                                ...
                                {this.state.boleto_recarga ? (
                                    <div>
                                        ...
                                        />
                                        <br /><br />
                                        <Button onClick={() => { this.setState({ carregar: false }); window.location.href = '/cartoes' }} className="btn-danger btnCartaoCancelar" >Cancelar</Button>
                                        <Button className="btnCartaoSucesso" style={{ float: 'right' }} type="button" onClick={() => this.gerar_boleto()} >Gerar Boleto</Button>
                                    </div>
                                ) : null}
                                {this.state.transf_recarga ? (
                                    <div>
                                        ...
                                        <input type="text"
                                            ...
                                            ref={(input) => { this.id_cartaoTransf = input; }}
                                        />

                                        <Button className="btn  btn-sm btn-success" style={{ marginLeft: 10, marginTop: '-5px' }} onClick={() => this.transf_cartao(this.state.id_cartaoTransf)}>Pesquisar</Button>
                                        <br /><br />

                                        {this.state.cancelar_buscar ? (<div>
                                            <Button onClick={() => { this.setState({ carregar: false }); window.location.href = '/cartoes' }} className="btn-danger btnCartaoCancelar" >Cancelar</Button>
                                        </div>) : null}
                                        ...
                                        {this.state.mostrar_valor ? (
                                            <div>
                                                ...
                                                />
                                                <Button onClick={() => { this.setState({ carregar: false }); window.location.href = '/cartoes' }} className="btn-danger btnCartaoCancelar" >Cancelar</Button>
                                                <Button className="btnCartaoSucesso" style={{ float: 'right' }} variant="success" onClick={() => this.gerar_pagamennto()}>Recarga</Button>
                                            </div>
                                        ) : null}
                    ...
                    {this.state.loading ? (
                        ...
                        <Modal centered animation={true} size="lg" show={this.state.modal_boleto} /* onHide={() => window.location.href = '/cartoes'} */ >
                            <Modal.Body>
                                <div>
                                    ...
                                    <Button onClick={() => { this.setState({ modal_boleto: false }); window.location.href = '/cartoes' }} className="btn-danger btnCartaoCancelar" >Cancelar</Button>
                                    <Button style={{ float: 'right' }} className="btnCartaoSucesso" varriant='success' onClick={() => this.gerar_pdf_boleto({ id: global.boleto.id_boleto_ciacard, idConta: global.boleto.idConta })} >Download PDF</Button>
                    ...
                    <Modal centered animation={true} size="lg" show={this.state.trocarSenha} onHide={() => window.location.href = '/cartoes'} >
                        <Modal.Body>
                            <h4>Para alterar a senha de acesso ao seu cartão {process.env.NOME_BANCO}, digite a nova senha e confirme.</h4> <hr />
                            ...
                            <Button onClick={() => { this.setState({ trocarSenha: false }); window.location.href = '/cartoes' }} className="btn-danger btnCartaoCancelar" >Cancelar</Button>
                            <Button className="btnCartaoSucesso" style={{ float: 'right' }} type="button" onClick={() => this.alterar_senha()} >Trocar senha</Button>
                        </Modal.Body>
                    </Modal>

                    {/* Modal Detalhes */}
                    <Modal centered animation={true} size="lg" show={this.state.detalhes} /* onHide={() => window.location.href = '/cartoes'} */ >
                        <Modal.Body>
                            <h3>
                                {/* Aqui você consegue bloquear ou desbloquear seu cartão {process.env.NOME_BANCO} */}
                                {i18n.t('escolha_cartao.bloquear', {nome_banco: process.env.NOME_BANCO})}
                            </h3>
                            <hr />
                            <br />
                            {/* <h4>Cartão: <u>{this.state.detalhes_numero_cartao}</u></h4> */}
                            <h4>{i18n.t('escolha_cartao.cartao')}<u>{this.state.detalhes_numero_cartao}</u></h4>
                            {/* <h4>ID: <u>{this.state.detalhes_id_cartao_numero}</u></h4> */}
                            <h4>{i18n.t('escolha_cartao.id')}<u>{this.state.detalhes_id_cartao_numero}</u></h4>
                            {/* <h4>Status: <u>{this.state.detalhes_cartao_status}</u> &nbsp; {this.state.cadeado}</h4> */}
                            <h4>{i18n.t('escolha_cartao.status')}<u>{this.state.detalhes_cartao_status}</u> &nbsp; {this.state.cadeado}</h4>
                            <br />
                            <hr />
                            <Button onClick={() => { this.setState({ detalhes: false }); window.location.href = '/cartoes' }} className="btn-danger btnCartaoCancelar" >Cancelar</Button>
                            <Button className="btnCartaoSucesso" style={{ float: 'right' }} type="button" onClick={() => this.desbloquear_bloquear_cartao(this.state.cartao_id_status)} >{this.state.operacao}</Button>
                        </Modal.Body>
                    </Modal>
